...
- (id)initInScrollView:(UIScrollView *)scrollView activityIndicatorView:(UIView *)activity
{
    ...
    if (self) {
        ...
        UILabel * lbTimeTitle = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, 30)];
        lbTimeTitle.text = @"Last update";
        lbTimeTitle.textAlignment = NSTextAlignmentCenter;
        lbTimeTitle.font = [UIFont systemFontOfSize:12.0f];
        lbTimeTitle.textColor = [UIColor whiteColor];
        lbTimeTitle.center = CGPointMake(self.frame.size.width*0.5f, -35);
        [self.scrollView addSubview:lbTimeTitle];
        
        _lbTime = [[UILabel alloc] initWithFrame:CGRectMake(lbTimeTitle.frame.origin.x, lbTimeTitle.frame.origin.y + lbTimeTitle.frame.size.height-10.0f, lbTimeTitle.frame.size.width, 30)];
        _lbTime.text = @"00-00-0000 00:00:00";
        _lbTime.textAlignment = NSTextAlignmentCenter;
        _lbTime.font = [UIFont systemFontOfSize:12.0f];
        _lbTime.textColor = [UIColor whiteColor];
        [self.scrollView addSubview:_lbTime];
        
        //update time
        NSDate * datenow = [NSDate date];
        NSDateFormatter *inputDateFormatter = [NSDateFormatter new];
        [inputDateFormatter setDateFormat:@"dd-MM-yyyy HH:mm:ss"];
        NSString *dateString = [inputDateFormatter stringFromDate:datenow];
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    ...
    if (_refreshing) {
        if (offset != 0) {
            ...
            _activity.center = CGPointMake(floor(self.frame.size.width*0.1f), MIN(offset + self.frame.size.height + floor(kOpenedViewHeight / 2), self.frame.size.height - kOpenedViewHeight/ 2));
    ...
    CGPoint bottomOrigin = CGPointMake(floor(self.bounds.size.width *0.1f), self.bounds.size.height - currentBottomPadding -currentBottomRadius);
    CGPoint topOrigin = CGPointZero;
    if (distance == 0) {
        topOrigin = CGPointMake(floor(self.bounds.size.width *0.1f), bottomOrigin.y);
    } else {
        topOrigin = CGPointMake(floor(self.bounds.size.width *0.1f), self.bounds.size.height + offset + currentTopPadding + currentTopRadius);
        if (percentage == 0) {
