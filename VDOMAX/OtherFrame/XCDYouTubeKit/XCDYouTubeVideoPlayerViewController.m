...
- (void) setVideoIdentifier:(NSString *)videoIdentifier
{
	...
	self.videoOperation = [[XCDYouTubeClient defaultClient] getVideoWithIdentifier:videoIdentifier completionHandler:^(XCDYouTubeVideo *video, NSError *error)
	{
		if (video)
		{
            if (_DownloadOnlyFlag) {
                _urlStreamDownload = video.streamURLs;
                
                if(self.delegate)[self.delegate didLoadYoutubeDownloadSoundOnly:_SoundOnlyFlag withInfo:_urlStreamDownload];
                //dont  Present view
...
- (void) stopWithError:(NSError *)error
{
    if(_DownloadOnlyFlag){
        if (self.delegate){
            [self.delegate didFailedLoadYoutubeDownloadInfoWithError:error];
        }
    }else{
