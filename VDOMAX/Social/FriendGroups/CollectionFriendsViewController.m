...
#import "CollectionSocialViewController.h"
...
@interface CollectionSocialViewController (){
    AppDelegate * app;
}

@end

@implementation CollectionSocialViewController
...
- (void)CreateCollectionViewWithRect:(CGRect )  rect{
    ...
    [collectionViewFlowLayout setItemSize:CGSizeMake(itemWidth, 1.4*itemWidth)];
    [collectionViewFlowLayout setHeaderReferenceSize:CGSizeMake(rect.size.width, 0)];
    [collectionViewFlowLayout setFooterReferenceSize:CGSizeMake(rect.size.width, 70)];
...
-(void)RequestLoadDataNewesSocialPage:(int) page{
    ...
    if (_socialtype == SocialTypeNewest) {
        urlString = [NSString stringWithFormat:@"%@/search/social?sort=N&page=%d",kIPAPIVDOMAX,page];
    }else if(_socialtype == SocialTypeMostFollower){
        urlString = [NSString stringWithFormat:@"%@/search/social?sort=M&page=%d",kIPAPIVDOMAX,page];
    }else{//SocialTypeAlphabet
        urlString = [NSString stringWithFormat:@"%@/search/social?sort=A&page=%d",kIPAPIVDOMAX,page];
    }
